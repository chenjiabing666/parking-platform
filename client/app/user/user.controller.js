...
$scope.exportExcel = function(){
                ...
                var url="http://localhost:8080/parking-server/"+"user/exportExcel.do?";

                // var modifyTopicUrl ="http://localhost:8080/parking-server/"+"user/exportExcel.do";// 接收上传文件的后台地址
...
$scope.deleteList = function(){
                ...
                var modifyTopicUrl ="http://localhost:8080/parking-server/"+"user/deleteUserBatch.do";// 接收上传文件的后台地址
                console.log($scope.selected);
                var temp = "";
...
/*        $scope.cancelClick = function(){
        ...
        //获取完成的任务
        $http.post('http://localhost:8080/parking-server/' + 'user/getTaskListBack.do',{},{params:{
        ...
        $scope.unbindingDecedent=function(decedentId){
                    ...
                    .title('是否确定解除绑定信息')
                                ...
                                .cancel('取消');
                                $mdDialog.show(confirm).then(function() {
            $http.post('http://localhost:8080/parking-server/' + 'user/unbindingDecedent.do',{},{params:{
            userId:$scope.userId,  //用户id
        ...
        $scope.resetPassword=function(userId){
                    ...
                    .title('是否确定重置密码')
                                ...
                                .cancel('取消');
                                $mdDialog.show(confirm).then(function() {
            $http.post('http://localhost:8080/parking-server/' + 'user/restPassword.do',{},{params:{
            userId:$scope.userId  //用户id
        }}).success( function (data){   
            if(data.code == 0){  //解除成功
                $scope.showAlert(data.successMessage);
    ...
    function DecedentCtrl($scope,$http,$location,$mdDialog,$timeout,$sce){
        ...
        //根据用户id获取其绑定的逝者信息
        $http.post('http://localhost:8080/parking-server/' + 'user/getDetailedInformation.do',{},{params:{
            userId:$scope.userId   //用户id
        }}).success( function (data){   
            if(data.code == 0){
...
$scope.deleteList = function(){
        ...
        $scope.deleteUser = function(id){
            $scope.showConfirm = function() {
                ...
                .title('是否确定删除该条用户信息')
                            ...
                            $mdDialog.show(confirm).then(function() {
                    // console.log('确定')
                    $http.post("http://localhost:8080/parking-server/"+"user/deleteById.do?",{},{params:{
                        userId:id
                    }}).success(function (data){
                        if(data.code == 0){
       ...
       function UserAddCtrl($scope,$http,$location,$mdDialog,$timeout){
        ...
        $scope.sendCode=function(mobile){
            console.log(mobile);
            $http.post("http://localhost:8080/parking-server/"+"user/genAuthCode.do?",{},{params:{
                        mobile:mobile
                    }}).success(function (data){
                        if(data.code == 0){
